...
test_that("classes are correct", {
   ...
   expect_is(sim_thomas_coords(sad1, n_mother_points = 0), "community")
   expect_is(sim_thomas_coords(sad1, n_mother_points = 100), "community")
   expect_is(sim_thomas_coords(sad1, n_mother_points = 4.2), "community")
   expect_is(sim_thomas_coords(sad1, n_mother_points = 1, points_per_cluster = 2), "community")
   expect_is(sim_thomas_coords(sad1, n_mother_points = 1, xmother=xmother, ymother=ymother), "community")
...
test_that("The function handles wrong n_mother_points parametres", {
   expect_error(sim_thomas_coords(sad1, n_mother_points = -1))  # n_mother_points is negative
   expect_error(sim_thomas_coords(sad1, n_mother_points = sample(size=100, -2:2, replace=T)))  # n_mother_points countains negative values
   expect_error(sim_thomas_coords(sad1, n_mother_points = rep(1, 4)))  # n_mother_points is too short
   expect_error(sim_thomas_coords(sad1, n_mother_points = rep(1, 1000)))  # n_mother_points is too long
})
...
if(FALSE) {
   sad2 <- sim_sad(4, 100)
   n_mother_points <-  c(1,-1,1,1)
   plot(sim_thomas_coords(sad2, n_mother_points = n_mother_points))
   ...
   # N_mother_points
   # if n_mother_points is numeric, it is truncated to integer by rThomas_rcpp
   # if n_mother_points > values of abund_vec, each mother point has 1 or 0 individual and the distribution is random
   ## Solved
   # if n_mother_points is of the same length as abund_vec and one or more value < 0, no clustering for this one but correct clustering for the others. No warning() -> NOW error
   # if n_mother_points is length 1 and value < 0, no clustering. No warning() -> NOW error
   # if n_mother_points countains one NA, other values are not taken into account and the function computes n_mother_points per species on its own. No warning. -> NOW error
   # if length(n_mother_points) < length(abund_vec), values are not taken into account and the function computes n_mother_points per species on its own. No warning -> NOW error
   # if length(n_mother_points) > length(abund_vec), only values 1 to length(abund_vec) are used. -> NOW error
}
