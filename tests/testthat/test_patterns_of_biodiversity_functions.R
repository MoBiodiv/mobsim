...
test_that("div_rect() - produces results comparable to vegan", {
  ...
  expect_equal(
    as.numeric(div1[3]),
    vegan::diversity(table(comm1$census$species), "shannon")
  )
  expect_equal(
    as.numeric(div1[4]),
    exp(vegan::diversity(table(comm1$census$species), "shannon"))
  )
  expect_equal(
    as.numeric(div1[5]),
    vegan::diversity(table(comm1$census$species), "simpson")
  )
  expect_equal(
    as.numeric(div1[6]),
    1 / (1 - vegan::diversity(table(comm1$census$species), "simpson"))
  )
})
...
test_that("div_rand_rect() - produces results comparable to vegan", {
  ...
  expect_equal(
    as.numeric(divRand[5]),
    vegan::diversity(table(comm1$census$species), "shannon")
  )
  expect_equal(
    as.numeric(divRand[7]),
    exp(vegan::diversity(table(comm1$census$species), "shannon"))
  )
  expect_equal(
    as.numeric(divRand[9]),
    vegan::diversity(table(comm1$census$species), "simpson")
  )
  expect_equal(
    as.numeric(divRand[11]),
    1 / (1 - vegan::diversity(table(comm1$census$species), "simpson"))
  )
})
...
test_that("divar() - condition handling", {
  ...
  expect_s3_class(
    divar(comm = comm1, prop_area = seq(0, 1, length = n_scales)),
    "data.frame"
  )
  expect_s3_class(divar(comm = comm1, prop_area = 1), "data.frame")
  expect_error(divar(
    comm = comm1,
    prop_area = c(NA, seq(0, 1, length = n_scales))
  ))
})
