...
for (i in 1:nsim) {
  ...
  com_uneven <- sim_poisson_community(
    s_pool = S0,
    n_sim = N0,
  ...
  src_uneven_mat[
    i,
    1:nrow(spec_curves_uneven)
  ] <- spec_curves_uneven$spec_rarefied
...
plot(
  ...
  type = "l",
  ylim = c(1, S0),
  main = "Change in no. of individuals",
  xlab = "",
  ylab = ""
)
...
polygon(
  c(1:N0, N0:1),
  c(src_ref_ci[1, ], rev(src_ref_ci[2, ])),
...
polygon(
  c(1:(N0 / 2), (N0 / 2):1),
  c(src_low_n_ci[1, ], rev(src_low_n_ci[2, ])),
...
polygon(
  c(1:N0, N0:1),
  c(sac_ref_ci[1, ], rev(sac_ref_ci[2, ])),
  col = adjustcolor("black", alpha.f = 0.1),
  border = NA
)
...
polygon(
  c(1:(N0 / 2), (N0 / 2):1),
  c(sac_low_n_ci[1, ], rev(sac_low_n_ci[2, ])),
  col = adjustcolor("red", alpha.f = 0.1),
  border = NA
)
...
polygon(
  c(1:N0, N0:1),
  c(src_uneven_ci[1, ], rev(src_uneven_ci[2, ])),
  col = adjustcolor("red", alpha.f = 0.1),
  border = NA
)
...
polygon(
  c(1:N0, N0:1),
  c(sac_uneven_ci[1, ], rev(sac_uneven_ci[2, ])),
  col = adjustcolor("red", alpha.f = 0.1),
  border = NA
)
...
polygon(
  c(1:N0, N0:1),
  c(src_agg_ci[1, ], rev(src_agg_ci[2, ])),
  col = adjustcolor("red", alpha.f = 0.1),
  border = NA
)
...
polygon(
  c(1:N0, N0:1),
  c(sac_agg_ci[1, ], rev(sac_agg_ci[2, ])),
  col = adjustcolor("red", alpha.f = 0.1),
  border = NA
)

mtext(
  "No. of expected species",
  side = 2,
  line = -1,
  outer = T,
  las = 0,
