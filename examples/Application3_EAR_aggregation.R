...
for (i in 1:nsim) {
  # random
  com_random <- sim_poisson_community(
    s_pool = S0,
    n_sim = N0,
  ...
  # sigma high
  com_sigma_high <- sim_thomas_community(
    s_pool = S0,
    n_sim = N0,
    sigma = sigma_high,
    sad_coef = list(meanlog = 3, sdlog = 1)
  ...
  # sigma low
  com_sigma_low <- sim_thomas_community(
    s_pool = S0,
    n_sim = N0,
    sigma = sigma_low,
    sad_coef = list(meanlog = 3, sdlog = 1)
  ...
  com_clust1 <- sim_thomas_community(
    s_pool = S0,
    n_sim = N0,
    sigma = sigma_high,
    mother_points = 1,
...
plot(
  ...
  type = "n",
  las = 1,
  xlab = "Proportion of area lost",
  ylab = "No. of species lost",
  main = "Endemics area relationships"
)
polygon(
  c(p_area, rev(p_area)),
  c(ear_random_ci[1, ], rev(ear_random_ci[2, ])),
...
polygon(
  c(p_area, rev(p_area)),
  c(ear_sigma_high_ci[1, ], rev(ear_sigma_high_ci[2, ])),
  col = adjustcolor(2, alpha.f = 0.1),
  border = NA
)
...
polygon(
  c(p_area, rev(p_area)),
  c(ear_sigma_low_ci[1, ], rev(ear_sigma_low_ci[2, ])),
  col = adjustcolor(3, alpha.f = 0.1),
  border = NA
)
...
legend(
  "topleft",
  legend = c("Random", "Large clusters", "Small clusters", "One large cluster"),
