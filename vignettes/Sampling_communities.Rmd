...
In addition to the simulation of spatially-explicit communities, `mobsim` provides
a function to generate samples from simulated or observed communities. The combination
of simulated data AND simulated sampling is powerful to test the validity and power
...
There is an option to exclude overlapping quadrats from the random sampling
design, which is shown here in two examples with different numbers and sizes
of the quadrats.
...
In addition to random design also transect can be sampled, which requires
a position for the lower left quadrat as well as x and y distances between
...
Finally, sampling quadrats can be arranged in a regular lattice.
