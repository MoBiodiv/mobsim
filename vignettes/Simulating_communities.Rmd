...
clusters (`n_mother_points`), and the mean number of individuals per cluster 
(`points_per_cluster`) the Thomas process can generate a large range of different
...
```{r, fig.width = 7, fig.height = 4}
comm3b <- sim_thomas_coords(abund_vec = abund1, sigma = 0.02, n_mother_points = 1)
par(mfrow = c(1,2))
plot(comm3)
plot(comm3b)
```
Third, we change the average number of points (i.e. individuals) per cluster using
the argument `points_per_cluster`.

```{r, fig.width = 7, fig.height = 4}
comm3c <- sim_thomas_coords(abund_vec = abund1, sigma = 0.02, points_per_cluster = 5)
par(mfrow = c(1,2))
plot(comm3)
plot(comm3c)
```
