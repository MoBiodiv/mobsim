...
sample_quadrats <- function(
  ...
  if (!is(comm, "community")) stop("comm has to be a community object")
  if (round(n_quadrats, 0) != n_quadrats)
    stop("n_quadrats has to be an integer")
  ...
  if (quadrat_size < community_size) {
    ...
    comm_tab <- mapply(
      ...
      xy_dat$x,
      xy_dat$y,
      MoreArgs = list(xsize = quadrat_size, ysize = quadrat_size, comm = comm)
    )
    ...
    spec_dat <- as.data.frame(matrix(
      comm_tab,
      nrow = 1L,
      byrow = TRUE,
      dimnames = list("site1", names(comm_tab))
    ))
  }
...
sampling_random_spatstat <- function(
  n_quadrats,
  min_dist,
  xmin,
  xmax,
  ...
  hc_mod <- list(
    cif = "hardcore",
    par = list(beta = n_quadrats, hc = min_dist),
    w = spatstat.geom::owin(c(xmin, xmax), c(ymin, ymax))
  )
  hc_points <- spatstat.random::rmh(
    model = hc_mod,
    start = list(n.start = n_quadrats),
  ...
  if (min(stats::dist(coords)) < min_dist)
    warning(
      "There are overlapping sampling squares in the design. Use less quadrats or smaller quadrat area."
    )
  return(coords)
}
...
#' @export
#'
sampling_random_bruteforce <- function(
  n_quadrats,
  min_dist,
  xmin,
  xmax,
  ...
  if (count > maxCount)
    warning(
      "Cannot find a sampling layout with no overlap.
...
#' @export
#'
sampling_random_overlap <- function(
  n_quadrats,
  min_dist,
  xmin,
  xmax,
