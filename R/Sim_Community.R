                                 ...
                                 fix.S = F)
{
   ...
   if (length(sigma) == 2){
      ...
      # # linear relationship between sigma and relabund
      # relabund <- abund.vec/sum(abund.vec)
      # range.abund <- max(relabund) - min(relabund)
      ...
      # a1 <- sigma[2] - b1*max(relabund)
      # sigma.vec <- a1 + b1*relabund
   }
   else {
