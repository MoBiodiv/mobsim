...
```
```{r plot_communities, fig.width=8, fig.height=4.5, eval=T}
par(mfrow = c(1, 2))
plot(comm_rand)
plot(comm_agg)
```
...
```{r plot_divar, fig.width=5, fig.height=5, eval = T}
...
```{r plot_sampling, fig.width=9, fig.height=5, eval = T}
par(mfrow = c(1, 2))
...
### Dynamic communities
This is a basic example which shows you how to make a community move in space:
